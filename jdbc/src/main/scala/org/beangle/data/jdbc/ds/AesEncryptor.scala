...
object AesEncryptor {
  ...
  def main(args: Array[String]): Unit = {
    if (args.length == 2) {
      val key = args(0)
      if (args(1).length == 32) {
        println(new AesEncryptor(key).decrypt(args(1)))
      } else {
        println(new AesEncryptor(key).encrypt(args(1)))
      }
    } else {
      println("USAGE: java AesEncryptor  key plain|encoded")
    }
  }
}

import org.beangle.data.jdbc.ds.AesEncryptor._
