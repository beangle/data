...
object Vendors {
   ...
   */
  val postgresql = Vendor("PostgreSQL",
  ...
  val mysql = Vendor("MySQL", Driver("mysql", "com.mysql.jdbc.jdbc2.optional.MysqlDataSource", "com.mysql.jdbc.Driver", "//<host>:<port>/<database_name>"))

  val mariadb = Vendor("MariaDB", Driver("mariadb", "org.mariadb.jdbc.MySQLDataSource", "org.mariadb.jdbc.Driver", "//<host>:<port>/<database_name>"))

  val h2 = Vendor("H2", Driver("h2", "org.h2.jdbcx.JdbcDataSource", "org.h2.Driver", "[<path>]<database_name>", "mem:<database_name>",
    "tcp://<server>[:<port>]/[<path>]<database_name>"))

  val db2 = Vendor("DB2", Driver("db2", "com.ibm.db2.jcc.DB2SimpleDataSource", "com.ibm.db2.jcc.DB2Driver", "//<host>[:<port>]/<database_name>",
    "<database_name>"))

  val derby = Vendor("Apache Derby", Driver("derby", "org.apache.derby.jdbc.ClientDataSource", "org.apache.derby.jdbc.ClientDriver",
    "[<subsubprotocol>:][<database_name>][;<attribute>=<value>]*"))

  val hsql = Vendor("HSQL Database Engine", Driver("hsqldb", "org.hsqldb.jdbc.JDBCDataSource", "org.hsqldb.jdbcDriver", "hsql://<host>:<port>",
    "file:<path>", "hsqls://<host>:<port>", "http://<host>:<port>", "https://<host>:<port>", "res:<database_name>"))

  //default port 1433
  val sqlserver = Vendor("Microsoft SQL Server",
  ...
  private val list = List(postgresql, oracle, mysql, mariadb, h2, db2, derby, hsql, sqlserver)
