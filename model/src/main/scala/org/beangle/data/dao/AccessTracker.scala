...
object AccessTracker {
  ...
  private[dao] val TrackerNamePostfix = "$Tracker"
  ...
  // ByteBuddy 仅在 JVM 模式需要；native-image 场景依赖构建期预生成的 tracker 类，
  // 因此这里保持惰性初始化，避免加载 AccessTracker 时触碰 ByteBuddy。
  private lazy val byteBuddy = buildByteBuddy()
  ...
  /** 尝试从 classpath 加载构建期预生成的 tracker 类（native-image 模式必须存在）
   *
   * 构建期由 [[AccessTrackerGenerator]] 依据 MappingModule 的绑定生成同名类，
   * 运行期优先使用预生成类，避免在 native-image 中执行 ByteBuddy 字节码生成。
   */
  private def loadPreGenerated(trackerClazzName: String): Option[Class[_]] = {
    try {
      val clazz = Class.forName(trackerClazzName, false, getClass.getClassLoader)
      if (classOf[AccessTracker].isAssignableFrom(clazz)) Some(clazz) else None
    } catch {
    ...
    if (null == existed || existed == classOf[AnyRef]) {
      ...
      Locks.withLock(lock) {
        ...
        if (null == existed) {
          ...
          existed = trackers.getOrElse(trackerClazzName, null)
          if (null == existed || existed == classOf[AnyRef]) {
            // native-image 模式下优先使用构建期预生成的 tracker 类
            loadPreGenerated(trackerClazzName) match {
              case Some(pre) =>
                trackers.put(trackerClazzName, pre)
                pre
              case None =>
  ...
  private def doGenerate(clazz: Class[_], trackerClazzName: String): Class[_] = {
    ...
    // 生成并加载类
    val dynamicType = buildUnloaded(clazz, trackerClazzName)
  ...
  /** 构建 tracker 类的字节码（不加载），供构建期工具生成 .class 文件
   *
   * @param clazz            origin class
   * @param trackerClazzName tracker class name
   * @return 未加载的字节码，可 saveIn 输出目录
   */
  private[dao] def buildUnloaded(clazz: Class[_], trackerClazzName: String): DynamicType.Unloaded[_] = {
