...
class HibernateEntityDao(sf: SessionFactory) extends EntityDao, Initializing {
  ...
  override def sessionFactory: EntityManagerFactory = {
    sf
  }

  override def openSession(): EntityManager = {
    SessionHelper.openSession(sf)
  }

  override def closeSession(em: EntityManager): Unit = {
    SessionHelper.closeSession(em)
  }
